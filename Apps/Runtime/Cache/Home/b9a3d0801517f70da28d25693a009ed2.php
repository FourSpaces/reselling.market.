...
<div id="wst-shortcut">
       ...
       <div class='wst-page-content'>
	       	<div id='tab' class="wst-tab-box">
				<ul class="wst-tab-nav">
			    	<li <?php if($speechType == "receive"): ?>class="on"<?php endif; ?>><a href="<?php echo U('Home/GoodsAppraises/getAppraisesList/',array('speechType'=>'receive'));?>">发送的留言</a></li>
			        <li  <?php if($speechType == "send"): ?>class="on"<?php endif; ?>>
			        	<a href="<?php echo U('Home/GoodsAppraises/getAppraisesList/',array('speechType'=>'send'));?>">收到的留言</a></li>
			    </ul>
		    <div>
        <table class='wst-list' style="font-size:13px;">
           <thead>
             <tr>
               ...
               <th width='80'><?php if($speechType == "receive"): ?>接收人<?php else: ?>发送人<?php endif; ?></th>
               <th width='*'>留言</th>
             </tr>
           </thead>
           <tbody>
            <?php if(is_array($appraiseList['root'])): $key1 = 0; $__LIST__ = $appraiseList['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$appraise): $mod = ($key1 % 2 );++$key1;?><tr style="border-bottom:1px solid #eeeeee;">
             	
                <td width='*'>
                    <div style='width:300px;'>
                    <div style='width:70px;float:left'>
					<a target='_blank' href="<?php echo U('Home/Goods/getGoodsDetails/',array('goodsId'=>$appraise['goodsId']));?>">
						<img src="/reselling.market/<?php echo ($appraise['goodsThums']); ?>" height="60" width="60"/>
					</a>
					</div>
					<div style='width:200px;float:left;line-height:25px;'>
					<div><?php echo ($appraise['goodsName']); ?></div>
					</div>
					</div>
				</td>
				<td><?php echo ($appraise['userName']); ?></td>
				<td >评价：<?php echo ($appraise["content"]); ?></td>
             </tr><?php endforeach; endif; else: echo "" ;endif; ?>
             <?php if($appraiseList['totalPage'] > 1): ?><tfoot>
             <tr>
    ...
    <script>
    <?php if($appraiseList['totalPage'] > 1): ?>$(document).ready(function(){
		laypage({
		    cont: 'wst-page-items',
		    pages: <?php echo ($appraiseList['totalPage']); ?>, //总页数
		    skip: true, //是否开启跳页
		    skin: '#e23e3d',
		    groups: 3, //连续显示分页数

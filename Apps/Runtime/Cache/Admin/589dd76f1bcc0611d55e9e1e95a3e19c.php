...
.amap_lib_placeSearch .poibox .poi-title {
   ...
   $(function () {
	   ...
	   $("#loginName").formValidator({onShow:"",onFocus:"会员账号应该为5-16字母、数字或下划线",onCorrect:"输入正确"}).inputValidator({min:5,max:16,onError:"会员账号应该为5-16字母、数字或下划线"}).regexValidator({
	       regExp:"username",dataType:"enum",onError:"会员账号格式错误"
			...
			<?php else: ?>.defaultPassed();<?php endif; ?>
	   $("#loginPwd").formValidator({
			onShow:"",onFocus:"登录密码长度应该为5-20位之间"
			}).inputValidator({
				min:5,max:50,onError:"登录密码长度应该为5-20位之间"
			});
		$("#userPhone").formValidator({onShow:"",onFocus:"手机号不能为空",onCorrect:"输入正确"}).inputValidator({min:11,max:11,onError:"你输入的手机号码非法,请确认"}).ajaxValidator({
			dataType : "json",
			async : true,
			url : "<?php echo U('Admin/Users/checkLoginKey');?>",
			success : function(data){
			...
			<?php else: ?>.defaultPassed();<?php endif; ?>
		$("#shopSn").formValidator({onShow:"",onFocus:"店铺编号不能超过20个字符",onCorrect:"输入正确"}).inputValidator({min:1,max:20,onError:"店铺编号不符合要求,请确认"})<?php if($object['shopId'] ==0 ): ?>;
		
			<?php else: ?>.defaultPassed();<?php endif; ?>
		$("#shopName").formValidator({onShow:"",onFocus:"店铺名称不能超过20个字符",onCorrect:"输入正确"}).inputValidator({min:1,max:20,onError:"店铺名称不符合要求,请确认"})<?php if($object['shopId'] ==0 ): ?>;
		
			<?php else: ?>.defaultPassed();<?php endif; ?>
		$("#userName").formValidator({onShow:"",onFocus:"请输入店主姓名",onCorrect:"输入正确"}).inputValidator({min:1,max:20,onError:"店主姓名不能为空,请确认"})<?php if($object['shopId'] ==0 ): ?>;
		
			<?php else: ?>.defaultPassed();<?php endif; ?>
		$("#shopCompany").formValidator({onShow:"",onFocus:"请输入公司名称",onCorrect:"输入正确"}).inputValidator({min:1,max:50,onError:"公司名称不能为空,请确认"})<?php if($object['shopId'] ==0 ): ?>;
		
			<?php else: ?>.defaultPassed();<?php endif; ?>
		$("#shopTel").formValidator({onShow:"",onFocus:"请输入店铺电话",onCorrect:"输入正确"}).inputValidator({min:1,max:50,onError:"店铺电话不能为空,请确认"})<?php if($object['shopId'] ==0 ): ?>;
		
			<?php else: ?>.defaultPassed();<?php endif; ?>
		$("#shopAddress").formValidator({onShow:"",onFocus:"请输入店铺地址",onCorrect:"输入正确"}).inputValidator({min:1,max:120,onError:"店铺地址不能为空,请确认"})<?php if($object['shopId'] ==0 ): ?>;
		
			<?php else: ?>.defaultPassed();<?php endif; ?>
		$("#areaId2").formValidator({onFocus:"请选择所属地区"}).inputValidator({min:1,onError: "请选择所属地区"});
		...
		*/
		...
		<?php if($object['shopId'] ==0 ): else: ?>
		   getAreaList("areaId2",<?php echo ($object["areaId1"]); ?>,0,<?php echo ($object["areaId2"]); ?>);<?php endif; ?>
		
		initTime('serviceStartTime','<?php echo ($object['serviceStartTime']); ?>');
		initTime('serviceEndTime','<?php echo ($object['serviceEndTime']); ?>');
		
		<?php if($object['shopId'] ==0 ): ?>//	document.getElementById("loginName").focus();
			<?php else: ?>
			getCommunitys();
		   	//document.getElementById("shopSn").focus();<?php endif; ?>
	...
	function yj_unformValidator(){
		...
		$("#areaId2").attr("disabled",true).unFormValidator(true); //解除校验
	...
	function reformValidator(){
		...
		$("#areaId2").attr("disabled",false).unFormValidator(false); //解除校验
   ...
   function ShopMapInit(option){
	   ...
	   AMap.event.addListener(shopMap,'click',function(e){
			...
			marker.setMap(shopMap);
			//$('#coordinates').val("经纬度：["+e.lnglat.getLng()+','+e.lnglat.getLat()+"]");
	   		openInfo(e.lnglat.getLng(),e.lnglat.getLat())
	   });
    ...
    function openInfo(Lng,Lat) {
        ...
        info.push("<div style=\"padding:0px 0px 0px 4px;\"><b>当前点经纬度</b>");
        info.push("经度 :"+Lng);
        info.push("维度 :"+Lat+"</div>");
        infoWindow = new AMap.InfoWindow({
            content: info.join("<br/>")  //使用默认信息窗体框样式，显示信息内容
        });
         // 设置缩放级别和中心点
        //shopMap.setZoomAndCenter(shopMap.getzoom(), [Lng, Lat]);
        var Center = Array(Lng,Lat); 
        //alert(Center);
        infoWindow.open(shopMap,Center);
    }

   function placeSearchs(Keyword){
   		var cityName =  $('#areaId1').find("option:selected").text();
   		var cityvalue =  $('#areaId1').find("option:selected").val();

   		//alert(cityName+'||'+cityvalue);
   		if(cityName=="请选择" || cityvalue==""){
   			alert('请先选择你所在的地区');
   		}
   ...
   function getAreaList(objId,parentId,t,id){
	   ...
	   if(t<1){
		   ...
		   if(t==1 && $('#areaId2').find("option:selected").text()!=''){
			   shopMap.setZoom(); //地图设置变焦
			   shopMap.setCity($('#areaId2').find("option:selected").text());
			   $('#showLevel').val(shopMap.getZoom());
		   }
	   }
		*/
	 	if(t==0 && $('#areaId1').find("option:selected").text()!=''){
			   shopMap.setZoom();
			   shopMap.setCity($('#areaId1').find("option:selected").text());
			   $('#showLevel').val(shopMap.getZoom());
		   }
	   var html = [];
   ...
   function edit(){
	   ...
	   if(params.relateAreaId=='0' && params.relateCommunityId=='0' && params.shopStatus!=-1){
		   Plugins.Tips({title:'信息提示',icon:'error',content:'请选择送货区域!',timeout:1000});
		   return;
	   }
	   //拒绝的时候要求写备注
   ...
   function addressLabels(){
   	  alert($("#shopAddress").val());
	  placeSearchs( $("#shopAddress").val());
   }
        ...
        <table class="table table-hover table-striped table-bordered wst-form">
           ...
           <tr style="height:80px;">
             ...
             <td>
	             <div id="preview_Filedata" style="margin-top: 70px;">
	             	<img id='preview' src='/reselling.market/<?php echo ($object["shopImg"]); ?>' ref='/reselling.market/<?php echo ($object["shopImg"]); ?>' style='width:152px;height:152px;max-width:152px;' <?php if($object['shopImg'] =='' ): ?>style='display:none'<?php endif; ?>/>
	             </div>
             </td>
           </tr>
           <tr>
           ...
           <tr>
            ...
            <tr>
             ...
             <td>
             <select id='areaId1' onchange='javascript:getAreaList("areaId2",this.value,0);getCommunitys();'>
             ...
             <select id='areaId2' onchange='javascript:placeSearchs($("#areaId2").find("option:selected").text())'>
               <option value=''>请选择</option>
             </select>
             </td>
           </tr>
           <tr>
             ...
             <td><input type='text' id='shopAddress' class="form-control wst-ipt" value='<?php echo ($object["shopAddress"]); ?>' maxLength='120' onblur='addressLabels()' />
           ...
           <tr id='shopMap' style='display:none'>
             <td>&nbsp;</td>
             <td>
             <div id="mapContainer" style='height:400px;width:90%;'>等待地图初始化...	
             	<div id="panel"></div>
             </div>
           ...
           <tr>
             <th align='right'>送货区域<font color='red'>*</font>：</th>
             <td>
           ...
           <tr class="hide" id='invoiceRemarkstr' <?php if($object['isInvoice'] == 0 ): ?>style='display:none'<?php endif; ?>>
